...
trait ProfileCvsTrait
{
    ...
    public function showcertificates(Request $request, $user_id)
    {
       
        
        $user = Certificate::where('user_id',$user_id)->get();
        ...
        foreach ($user as $certificate):
        
        
   
        $html .= '<tr id="certificate_' . $certificate->id . '">
        <td>' . $certificate->certificate_name . '</td>
        <td><a href="javascript:;" onclick="showcertificateEditModal(' . $certificate->id . ');"  class="text text-warning">' . __('Edit') . '</a>&nbsp;|&nbsp;<a href="javascript:;" onclick="delete_certificate(' . $certificate->id . ');" class="text text-danger">' . __('Delete') . '</a></td>
    ...
    public function getProfilecertificateForm(Request $request, $user_id)
    {
        $user = User::find($user_id);
        $returnHTML = view('admin.user.forms.certifications.certificate_modal')->with('user', $user)->render();
        return response()->json(array('success' => true, 'html' => $returnHTML));
    }
    ...
    public function storecertificate(CertificateFormRequest $request, $user_id)
    {
        ...
        $certificate = $this->assignValues1($certificate, $request, $user_id);
        $certificate->save();

        $returnHTML = view('admin.user.forms.certifications.certificate_thanks')->render();
        return response()->json(array('success' => true, 'status' => 200, 'html' => $returnHTML), 200);
    }
    
    public function storefrontcertificates(CertificateFormRequest $request, $id)
    {

        $certificate = new Certificate();
        $certificate = $this->assignValues2($certificate, $request, $id);
        $certificate->save();

     
        return response()->json(array('success' => true, 'status' => 200), 200);
    }
    private function assignValues2($certificate, $request, $id)
    {
        $certificate->user_id = $id;
        $certificate->certificate_name = $request->input('certificate_name');
        $certificate->certificate_agency = $request->input('certificate_agency');
        $certificate->year_of_passing = $request->input('year_of_passing');
        $certificate->month_of_passing = $request->input('month_of_passing');
    ...
    private function assignValues1($certificate, $request, $user_id)
    {
    ...
    {
        $id = $request->input('id');
        echo $this->removecertificate($id);
    }

    private function removecertificate($id)
    {
        try {
    ...
    public function getcertificateEditForm(Request $request, $user_id)
    {
        $certificate_id = $request->input('certificate_id');
        $certificate = Certificate::find($certificate_id);
        $user = User::find($user_id);
        $returnHTML = view('admin.user.forms.certifications.certificate_edit_modal')
                ->with('user', $user)
    ...
    public function getfrontcertificateEditForm(Request $request, $id)
    {
        $certificate_id = $request->input('certificate_id');
        $certificate = Certificate::find($certificate_id);
        $user = User::find($id);
        $returnHTML = view('user.forms.certificates.certificate_edit_modal')
                ->with('user', $user)
                ->with('certificate', $certificate)
                ->render();
        return response()->json(array('success' => true, 'html' => $returnHTML));
    ...
    public function updatefrontcertificates(CertificateFormRequest $request,$id)
    {

        $certificate = Certificate::find($request->input('certificate_id'));
        $certificate = $this->assignValues1($certificate, $request, $id);
        $certificate->update();

       // $returnHTML = view('admin.user.forms.certifications.certificate_edit_thanks')->render();
        return response()->json(array('success' => true, 'status' => 200), 200);
    }
    
    public function updatecertificate(Request $request, $certificate_id, $user_id)
    {
