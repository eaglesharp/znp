...

{
...

    {
        ...
        $lang=Profilelanguage::where('user_id',$user_id)->get();
        ...
        if (isset($user) && count($user->profileLanguages)):
            ...
            foreach ($user->profileLanguages as $language):
                ...
                $data=unserialize($language->language_level);



                if (in_array(1, $data) && in_array(2, $data) && in_array(3, $data)){

                    $data = "Read, Write, Speak";

                }

                elseif (in_array(1, $data) && in_array(2, $data)){
                ...
                elseif (in_array(1, $data) && in_array(3, $data)){
                ...
                $html .= '<tr id="language_' . $language->id . '">
						...
						<td><span class="text text-success">' . $language->language . '</span></td>
...

    {
     ...
     //  return dd($request->language_level);
    ...
    public function storeProfileLanguagefront(ProfileLanguageFormRequest $request, $user_id)
    ...
    public function updateFrontProfileLanguage(ProfileLanguageFormRequest $request,$id)

    {



        $profileLanguage=ProfileLanguage::where('user_id',$id)->first();
       ...
       // return $language;
        ...
        // $profileLanguage = ProfileLanguage::find($user_id);

        $profileLanguage->user_id = $id;

        $profileLanguage->language_type = $request->input('language_type');      

        $profileLanguage->language = $request->input('language');

       

            $profileLanguage->language_level = serialize($request->input('language_level'));
