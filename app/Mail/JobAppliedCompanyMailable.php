<?php

namespace App\Mail;

use App\ProfileDetails;
use App\ProfileNop;
use App\ProfileSummary;
use Illuminate\Mail\Mailable;
use Illuminate\Queue\SerializesModels;

class JobAppliedCompanyMailable extends Mailable
{

    use SerializesModels;

    public $job;
    public $jobApply;

    /**
     * Create a new message instance.
     *
     * @return void
     */
    public function __construct($job, $jobApply)
    {
        $this->job = $job;
        $this->jobApply = $jobApply;
    }

    /**
     * Build the message.
     *
     * @return $this
     */
    public function build()
    {
        $company = $this->job->getCompany();
        $user = $this->jobApply->getUser();
        $details = ProfileDetails::where('user_id',$user->id)->first();
        $pdetails = ProfileSummary::where('user_id',$user->id)->first();
        $nop = ProfileNop::where('user_id',$user->id)->first();


        return $this->from(config('mail.recieve_to.address'), config('mail.recieve_to.name'))
                        ->replyTo(config('mail.recieve_to.address'), config('mail.recieve_to.name'))
                        ->to($company->email, $company->name)
                        ->subject('Job seeker named "' . $user->first_name . '" has applied on job "' . $this->job->job_title)
                        ->view('emails.job_applied_company_message')
                        ->with(
                                [
                                    'job_title' => $this->job->job_title,
                                    'company_name' => $company->name,
                                    'user_name' => $user->first_name,
                                    'user' => $user,
                                    'details' => $details,
                                    'pdetails' => $pdetails,
                                    'nop' => $nop,
                                    'job_date' => $this->job->created_at,
                                    'user_link' => route('user.profile', $user->id),
                                    'job_link' => route('view.applicants.list', [$this->job->id])
                                ]
        );
    }

}
