...
{{--        <button class="btn purple btn-outline sbold" onclick="showProfileGapModal();"> Add Gap </button>--}}
{{--    </div>--}}
    {{-- <div class="row">
        <div class="col-md-12">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="portlet light portlet-fit bordered">
                <div class="portlet-title">
                    <div class="caption"> <i class=" icon-layers font-green"></i> <span class="caption-subject font-green bold uppercase">Gap</span> </div>
                </div>
             
                <div class="portlet-body"><div class="row" id="gap_div"></div></div>
            </div>
        </div>
    </div>
</div>
<div class="modal fade bs-modal-lg" id="add_experience_modal" tabindex="-1" role="dialog" aria-hidden="true"></div>
<div class="modal fade bs-modal-lg" id="add_gap_modal" tabindex="-1" role="dialog" aria-hidden="true"></div>
@push('css')
<style type="text/css">
    .datepicker>div {
        display: block;
...
<script type="text/javascript">
    ...
    function loadProfileGapForm(){
    $.ajax({
    type: "POST",
            url: "{{ route('get.profile.gap.form', $user->id) }}",
            data: {"_token": "{{ csrf_token() }}"},
            datatype: 'json',
            success: function (json) {
            $("#add_gap_modal").html(json.html);
    ...
    function showProfileGAPEditModal(profile_experience_id){
    $("#add_gap_modal").modal();
    loadProfileGapEditForm(profile_experience_id);
    }
    ...
    function loadProfileGapEditForm(profile_experience_id){
    $.ajax({
    type: "POST",
            url: "{{ route('get.profile.gap.edit.form', $user->id) }}",
            data: {"profile_experience_id": profile_experience_id, "_token": "{{ csrf_token() }}"},
            datatype: 'json',
            success: function (json) {
            $("#add_gap_modal").html(json.html);
    ...
    function submitProfileGAPForm() {
    var form = $('#add_edit_profile_GAP');
    $.ajax({
    url     : form.attr('action'),
            type    : form.attr('method'),
            data    : form.serialize(),
    ...
    function delete_profile_GAP(id) {
  
    if (confirm('Are you sure! you want to delete?')) {
    $.post("{{ route('delete.profile.gap') }}", {id: id, _method: 'DELETE', _token: '{{ csrf_token() }}'})
            .done(function (response) {
            if (response == 'ok')
            {
            $('#experience_' + id).remove();
    ...
    function showGAP()
    {
    $.post("{{ route('show.profile.gap', $user->id) }}", {user_id: {{$user->id}}, _method: 'POST', _token: '{{ csrf_token() }}'})
            .done(function (response) {
            $('#gap_div').html(response);
            });
    }
